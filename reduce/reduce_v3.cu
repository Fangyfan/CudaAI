...
__device__ __forceinline__ void warp_reduce(float* shared, int tid) {
    float val = shared[tid] + shared[tid + warpSize];
...
__global__ void reduce_v3(float* g_idata, float* g_odata) {
    ...
    for (int stride = (blockDim.x >> 1); stride > warpSize; stride >>= 1) {
...
int main() {
    ...
    for (int i = 0; i < 10; ++i) {
        reduce_v3<<<block_num, thread_num>>>(d_data, d_out);
        reduce_v3<<<1, block_num / 2>>>(d_out, d_final_out);
    }
