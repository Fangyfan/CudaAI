...
#pragma unroll
        ...
        for (int k = 0; k < d; k += Bd) {
            ...
            LDST_32_BITS(cur0_h2) = LDST_32_BITS(RC[0]);
            LDST_32_BITS(cur1_h2) = LDST_32_BITS(RC[1]);
            LDST_32_BITS(cur2_h2) = LDST_32_BITS(RC[2]);
            LDST_32_BITS(cur3_h2) = LDST_32_BITS(RC[3]);
...
constexpr float kAtolHalf  = 5e-2f;
constexpr float kRtolHalf  = 5e-2f;

__global__ void fill_float_kernel(float* ptr, float value, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) ptr[idx] = value;
}

void fill_device_float(float* ptr, float value, int n, cudaStream_t stream) {
    constexpr int kBlock = 256;
    int grid = (n + kBlock - 1) / kBlock;
    fill_float_kernel<<<grid, kBlock, 0, stream>>>(ptr, value, n);
}
...
ErrorStats compare_against_ref(const std::vector<float>& ref,
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (!std::isfinite(r) || !std::isfinite(v)) {
            st.passed = false;
            if (st.first_bad_idx < 0) {
                st.first_bad_idx = i;
                st.first_ref = r;
                st.first_val = v;
            }
            st.max_abs_err = std::numeric_limits<float>::infinity();
            st.max_rel_err = std::numeric_limits<float>::infinity();
            continue;
        }

        const float abs_err = std::fabs(r - v);
        const float rel_err = abs_err / (std::fabs(r) + 1e-6f);
        st.max_abs_err = std::max(st.max_abs_err, abs_err);
        st.max_rel_err = std::max(st.max_rel_err, rel_err);
...
void reset_fa1_buffers(float* O, float* l, float* m,
                       int total_qkv, int total_rows,
                       cudaStream_t stream) {
    CHECK_CUDA(cudaMemsetAsync(O, 0, static_cast<size_t>(total_qkv) * sizeof(float), stream));
    CHECK_CUDA(cudaMemsetAsync(l, 0, static_cast<size_t>(total_rows) * sizeof(float), stream));
    fill_device_float(m, -INFINITY, total_rows, stream);
    CHECK_CUDA(cudaGetLastError());
}

void reset_half_output(half* O, int total_qkv, cudaStream_t stream) {
    CHECK_CUDA(cudaMemsetAsync(O, 0, static_cast<size_t>(total_qkv) * sizeof(half), stream));
}

template <typename ResetFn, typename LaunchFn>
float benchmark_kernel(ResetFn&& reset_fn,
                       LaunchFn&& launch_fn,
                       cudaStream_t stream,
                       int warmup_iters,
                       int bench_iters) {
...
std::vector<float> half_vector_to_float(const std::vector<half>& in) {
...
std::vector<half> float_vector_to_half(const std::vector<float>& in) {
...
template <typename ResetFn, typename LaunchFn>
BenchResult run_case_float(const std::string& name,
    ...
    res.avg_ms = benchmark_kernel(reset_fn, launch_fn, stream, warmup__iters, bench__iters);
    ...
    std::vector<float> host_out(total_qkv);
    CHECK_CUDA(cudaMemcpy(host_out.data(), d_O,
                          static_cast<size_t>(total_qkv) * sizeof(float),
                          cudaMemcpyDeviceToHost));
    res.err = compare_against_ref(ref_host, host_out, atol, rtol);
    return res;
}

template <typename ResetFn, typename LaunchFn>
BenchResult run_case_half(const std::string& name,
                          const std::string& ref_name,
                          ResetFn&& reset_fn,
                          LaunchFn&& launch_fn,
                          half* d_O,
    ...
    std::vector<half> host_out_half(total_qkv);
    CHECK_CUDA(cudaMemcpy(host_out_half.data(), d_O,
                          static_cast<size_t>(total_qkv) * sizeof(half),
                          cudaMemcpyDeviceToHost));
    std::vector<float> host_out = half_vector_to_float(host_out_half);
    res.err = compare_against_ref(ref_host, host_out, atol, rtol);
    return res;
}

void print_result(const BenchResult& r, float baseline_ms) {
    std::cout << std::left << std::setw(16) << r.name
              << std::left << std::setw(16) << r.ref_name
              << std::right << std::setw(12) << std::fixed << std::setprecision(4) << r.avg_ms
              << std::setw(12) << std::setprecision(3) << (baseline_ms / r.avg_ms)
              << std::setw(10) << (r.err.passed ? "PASS" : "FAIL")
              << std::setw(14) << std::scientific << std::setprecision(3) << r.err.max_abs_err
              << std::setw(14) << r.err.max_rel_err;

    if (!r.err.passed) {
        std::cout << "  first_bad_idx=" << r.err.first_bad_idx
                  << " ref=" << r.err.first_ref
                  << " val=" << r.err.first_val;
    }
    std::cout << "\n";
...
int main() {
    ...
    const int total_qkv = batch * heads * N * d;
    const int total_rows = batch * heads * N;
    const int total_scores = batch * heads * N * N;

    const size_t qkv_float_bytes = static_cast<size_t>(total_qkv) * sizeof(float);
    const size_t qkv_half_bytes  = static_cast<size_t>(total_qkv) * sizeof(half);
    const size_t row_bytes = static_cast<size_t>(total_rows) * sizeof(float);
    const size_t score_bytes = static_cast<size_t>(total_scores) * sizeof(float);

    std::vector<float> h_Q(total_qkv);
    std::vector<float> h_K(total_qkv);
    std::vector<float> h_V(total_qkv);
    ...
    // half kernels use the exact same initialized data, quantized to half once outside timing.
    std::vector<half> h_Q_half = float_vector_to_half(h_Q);
    std::vector<half> h_K_half = float_vector_to_half(h_K);
    std::vector<half> h_V_half = float_vector_to_half(h_V);
    std::vector<float> h_Q_half_fp32 = half_vector_to_float(h_Q_half);
    std::vector<float> h_K_half_fp32 = half_vector_to_float(h_K_half);
    std::vector<float> h_V_half_fp32 = half_vector_to_float(h_V_half);

    float *d_Q = nullptr, *d_K = nullptr, *d_V = nullptr;
    float *d_Q_halfref = nullptr, *d_K_halfref = nullptr, *d_V_halfref = nullptr;
    ...
    CHECK_CUDA(cudaMalloc(&d_V, qkv_float_bytes));
    CHECK_CUDA(cudaMalloc(&d_Q_halfref, qkv_float_bytes));
    CHECK_CUDA(cudaMalloc(&d_K_halfref, qkv_float_bytes));
    CHECK_CUDA(cudaMalloc(&d_V_halfref, qkv_float_bytes));
    CHECK_CUDA(cudaMalloc(&d_O, qkv_float_bytes));
    CHECK_CUDA(cudaMalloc(&d_S, score_bytes));
    CHECK_CUDA(cudaMalloc(&d_P, score_bytes));
    CHECK_CUDA(cudaMalloc(&d_l, row_bytes));
    CHECK_CUDA(cudaMalloc(&d_m, row_bytes));

    CHECK_CUDA(cudaMalloc(&d_Qh, qkv_half_bytes));
    CHECK_CUDA(cudaMalloc(&d_Kh, qkv_half_bytes));
    CHECK_CUDA(cudaMalloc(&d_Vh, qkv_half_bytes));
    CHECK_CUDA(cudaMalloc(&d_Oh, qkv_half_bytes));

    CHECK_CUDA(cudaMemcpy(d_Q, h_Q.data(), qkv_float_bytes, cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(d_K, h_K.data(), qkv_float_bytes, cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(d_V, h_V.data(), qkv_float_bytes, cudaMemcpyHostToDevice));

    CHECK_CUDA(cudaMemcpy(d_Q_halfref, h_Q_half_fp32.data(), qkv_float_bytes, cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(d_K_halfref, h_K_half_fp32.data(), qkv_float_bytes, cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(d_V_halfref, h_V_half_fp32.data(), qkv_float_bytes, cudaMemcpyHostToDevice));

    CHECK_CUDA(cudaMemcpy(d_Qh, h_Q_half.data(), qkv_half_bytes, cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(d_Kh, h_K_half.data(), qkv_half_bytes, cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(d_Vh, h_V_half.data(), qkv_half_bytes, cudaMemcpyHostToDevice));
    ...
    auto reset_noop = [&]() {};
    auto reset_fa1 = [&]() {
        reset_fa1_buffers(d_O, d_l, d_m, total_qkv, total_rows, stream);
    };
    auto reset_half = [&]() {
        reset_half_output(d_Oh, total_qkv, stream);
    };

    auto launch_baseline = [&]() {
        baseline::launch_attention_baseline(d_Q, d_K, d_V, d_O, d_S, d_P,
                                            batch, heads, N, d, handle, stream);
    };
    auto launch_baseline_halfref = [&]() {
        baseline::launch_attention_baseline(d_Q_halfref, d_K_halfref, d_V_halfref, d_O, d_S, d_P,
                                            batch, heads, N, d, handle, stream);
    };

    BenchResult baseline_res;
    baseline_res.name = "baseline";
    baseline_res.ref_name = "self";
    baseline_res.avg_ms = benchmark_kernel(reset_noop, launch_baseline,
                                           stream, warmup__iters, bench__iters);

    launch_baseline();
    CHECK_CUDA(cudaGetLastError());
    CHECK_CUDA(cudaStreamSynchronize(stream));

    std::vector<float> h_baseline(total_qkv);
    CHECK_CUDA(cudaMemcpy(h_baseline.data(), d_O, qkv_float_bytes, cudaMemcpyDeviceToHost));
    baseline_res.err = compare_against_ref(h_baseline, h_baseline, kAtolFloat, kRtolFloat);

    // For half-input kernels, compare against a baseline run on the same half-quantized inputs,
    // converted back to float, so input quantization error is not mistaken for kernel error.
    launch_baseline_halfref();
    CHECK_CUDA(cudaGetLastError());
    CHECK_CUDA(cudaStreamSynchronize(stream));

    std::vector<float> h_baseline_half_input(total_qkv);
    CHECK_CUDA(cudaMemcpy(h_baseline_half_input.data(), d_O, qkv_float_bytes, cudaMemcpyDeviceToHost));

    std::vector<BenchResult> results;
    results.push_back(baseline_res);

    results.push_back(run_case_float(
        "fa1_version3", "baseline_fp32",
        reset_fa1,
        [&]() {
            fa1_version3::launch_flash_attention_v1(d_Q, d_K, d_V, d_O, d_l, d_m,
                                                 batch, heads, N, d, stream);
        },
        d_O, h_baseline, total_qkv, stream, kAtolFloat, kRtolFloat));

    results.push_back(run_case_float(
        "fa2_version1", "baseline_fp32",
        reset_noop,
        [&]() {
            fa2_version1::launch_flash_attention_v2(d_Q, d_K, d_V, d_O,
                                                    batch, heads, N, d, stream);
        },
        d_O, h_baseline, total_qkv, stream, kAtolFloat, kRtolFloat));

    results.push_back(run_case_float(
        "fa2_version2", "baseline_fp32",
        reset_noop,
        [&]() {
            fa2_version2::launch_flash_attention_v2(d_Q, d_K, d_V, d_O,
                                                    batch, heads, N, d, stream);
        },
        d_O, h_baseline, total_qkv, stream, kAtolFloat, kRtolFloat));

    results.push_back(run_case_float(
        "fa2_version3", "baseline_fp32",
        reset_noop,
        [&]() {
            fa2_version3::launch_flash_attention_v2(d_Q, d_K, d_V, d_O,
                                                    batch, heads, N, d, stream);
        },
        d_O, h_baseline, total_qkv, stream, kAtolFloat, kRtolFloat));

    results.push_back(run_case_half(
        "fa2_version4", "baseline_halfin",
        reset_half,
        [&]() {
            fa2_version4::launch_flash_attention_v2(d_Qh, d_Kh, d_Vh, d_Oh,
                                                    batch, heads, N, d, stream);
        },
        d_Oh, h_baseline_half_input, total_qkv, stream, kAtolHalf, kRtolHalf));

    results.push_back(run_case_half(
        "fa2_version5", "baseline_halfin",
        reset_half,
        [&]() {
            fa2_version5::launch_flash_attention_v2(d_Qh, d_Kh, d_Vh, d_Oh,
                                                    batch, heads, N, d, stream);
        },
        d_Oh, h_baseline_half_input, total_qkv, stream, kAtolHalf, kRtolHalf));

    results.push_back(run_case_half(
        "fa2_version6", "baseline_halfin",
        reset_half,
        [&]() {
            fa2_version6::launch_flash_attention_v2(d_Qh, d_Kh, d_Vh, d_Oh,
                                                    batch, heads, N, d, stream);
        },
        d_Oh, h_baseline_half_input, total_qkv, stream, kAtolHalf, kRtolHalf));

    results.push_back(run_case_half(
        "fa2_version7", "baseline_halfin",
        reset_half,
        [&]() {
            fa2_version7::launch_flash_attention_v2(d_Qh, d_Kh, d_Vh, d_Oh,
                                                    batch, heads, N, d, stream);
        },
        d_Oh, h_baseline_half_input, total_qkv, stream, kAtolHalf, kRtolHalf));
    ...
    std::cout << "Config: "
              ...
              << ", bench=" << bench__iters << "\n";
    std::cout << "Timing baseline: fp32 baseline on original fp32 inputs\n";
    std::cout << "Correctness ref for half kernels: baseline on the same half-quantized inputs\n\n";

    std::cout << std::left << std::setw(16) << "kernel"
              << std::left << std::setw(16) << "ref"
    ...
    const float baseline_ms = baseline_res.avg_ms;
    for (const auto& r : results) {
        print_result(r, baseline_ms);
    }
    ...
    CHECK_CUDA(cudaFree(d_Q_halfref));
    CHECK_CUDA(cudaFree(d_K_halfref));
    CHECK_CUDA(cudaFree(d_V_halfref));
