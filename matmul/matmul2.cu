...
int main() {
    ...
    // 打开 csv 文件
    std::ofstream ofs("sgemm_benchmark_v3.csv");
    ...
    for (int N : sizes) {
        ...
        try {
            ...
            for (int i = 0; i < warmup_times; ++i) {
                checkCublasError(cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, N, N, &alpha, d_B, N, d_A, N, &beta, d_C_v3, N), 
            ...
            // 拷贝 cublas 结果
            checkCudaError(cudaMemcpy(C_cublas, d_C_v3, size, cudaMemcpyDeviceToHost), "cudaMemcpy C_cublas failed");
            ...
            // warm up
            for (int i = 0; i < warmup_times; ++i) {
                mysgemm_v3<256, 128, 128, 8, 8, 8><<<gridDim, blockDim>>>(N, N, N, alpha, d_A, d_B, beta, d_C_v3);
            }
            cudaDeviceSynchronize();

            // v3 kenrel launch
            float v3_time_ms = 0.0f;
            checkCudaError(cudaEventRecord(start), "v3 cudaEventRecord(start) failed");
            for (int i = 0; i < repeat_times; ++i) {
                mysgemm_v3<256, 128, 128, 8, 8, 8><<<gridDim, blockDim>>>(N, N, N, alpha, d_A, d_B, beta, d_C_v3);
            }
            checkCudaError(cudaEventRecord(end), "v3 cudaEventRecord(end) failed");
            checkCudaError(cudaEventSynchronize(end), "v3 cudaEventSynchronize(end) failed");
            checkCudaError(cudaEventElapsedTime(&v3_time_ms, start, end), "v3 cudaEventElapsedTime failed");

            // 拷贝 v3 结果
            checkCudaError(cudaMemcpy(C_v3, d_C_v3, size, cudaMemcpyDeviceToHost), "cudaMemcpy C_v3 failed");
            ...
            float v3_gflops = repeat_times * 2.0f * N * N * N / (v3_time_ms * 1e6f);

            // 写入 CSV
            ofs << N << "," << cublas_gflops << "," << v3_gflops << "," << error_count << ",";
            ofs << std::fixed << std::setprecision(2) << (100 * v3_gflops / cublas_gflops) << "%" << std::endl;
